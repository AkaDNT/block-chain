...
app.post('/api/initialize/pool', async (req, res) => {
  try {
    ...
    if (!ammAddress || !stockTokenAddress || !baseTokenAddress || !initialStock || !initialBase) {
      return res.status(400).json({
    ...
    const stockToken = new ethers.Contract(stockTokenAddress, StockTokenArtifact.abi, wallet)
    const baseToken = new ethers.Contract(baseTokenAddress, StockTokenArtifact.abi, wallet)
